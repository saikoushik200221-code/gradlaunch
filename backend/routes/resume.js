...
router.post('/upload', upload.single('resume'), async (req, res) => {
  try {
    ...
    const parseResp = await model.generateContent(PARSE_PROMPT + rawText);
    const parsedText = parseResp.response.text().trim().replace(/^\s*```json\s*|\s*```\s*$/g, '');
...
router.post('/tailor', async (req, res) => {
  try {
    ...
    const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });
    const resp = await model.generateContent(TAILOR_PROMPT(resume, job_description));
    const rawOutput = resp.response.text().trim().replace(/^\s*```json\s*|\s*```\s*$/g, '');
