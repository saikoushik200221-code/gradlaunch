...
function JobMatches({ jobs, loading, onRefresh, onOpenJob, onAddToTracker }) {
  ...
  if (sortBy === "match") filtered = [...filtered].sort((a, b) => b.match - a.match);
  if (sortBy === "recent") filtered = [...filtered].sort((a, b) => a.posted.localeCompare(b.posted));

  return (
    <div>
      <TopBar
        title="Job Matches"
        subtitle={`${filtered.length} roles ranked by your resume`}
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 18 }}>
        <div style={{ flex: 1, position: "relative" }}>
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by title, company, skill…" style={{ width: "100%", padding: "11px 14px 11px 40px", borderRadius: 10, background: C.surface, border: `1px solid ${C.border}`, color: C.text, fontSize: 13.5, fontFamily: "DM Sans, sans-serif", outline: "none", boxSizing: "border-box" }} onFocus={(e) => e.target.style.borderColor = C.accent} onBlur={(e) => e.target.style.borderColor = C.border} />
        </div>
        <select value={filterRemote} onChange={(e) => setFilterRemote(e.target.value)} style={{ padding: "11px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.border}`, color: C.text, fontSize: 13, fontFamily: "DM Sans, sans-serif", outline: "none", cursor: "pointer" }}>
          <option>All</option><option>Remote</option><option>Hybrid</option><option>Onsite</option>
        </select>
        <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} style={{ padding: "11px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.border}`, color: C.text, fontSize: 13, fontFamily: "DM Sans, sans-serif", outline: "none", cursor: "pointer" }}>
          <option value="match">Best match</option><option value="recent">Most recent</option>
        </select>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {filtered.map((job) => (
          <div key={job.id} onClick={() => onOpenJob(job)} style={{ padding: 18, borderRadius: 12, background: C.card, border: `1px solid ${C.border}`, cursor: "pointer", transition: "all 0.15s", position: "relative", overflow: "hidden" }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = C.borderHi; e.currentTarget.style.transform = "translateY(-1px)"; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.transform = "translateY(0)"; }}>
            {job.match >= 90 && <div style={{ position: "absolute", top: 0, right: 0, width: 120, height: 120, background: `radial-gradient(circle at top right, ${C.accentGlow} 0%, transparent 70%)`, pointerEvents: "none" }} />}
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 12, marginBottom: 4 }}>
                  <div>
                    <h3 style={{ margin: 0, fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: 17, color: C.text, letterSpacing: "-0.01em" }}>{job.title}</h3>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 5, color: C.textDim, fontSize: 12.5 }}>
                      <span style={{ fontWeight: 600, color: C.text }}>{job.company}</span>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 12, fontSize: 12, color: C.textDim }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 4, color: C.green, fontWeight: 600 }}><DollarSign size={12} />{job.salary}</span>
                  <span style={{ color: C.border }}>·</span>
                  <span>{job.remote}</span>
                  <span style={{ color: C.border }}>·</span>
                  <span>{job.applicants} applicants</span>
                  {job.insider > 0 && (<><span style={{ color: C.border }}>·</span><span style={{ display: "flex", alignItems: "center", gap: 4, color: C.purple }}><Users size={11} />{job.insider} insider{job.insider > 1 ? "s" : ""}</span></>)}
                </div>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
                  <button onClick={(e) => { e.stopPropagation(); onAddToTracker(job); }} style={{ padding: "7px 14px", borderRadius: 8, background: C.accent, border: "none", color: C.bg, fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: "DM Sans, sans-serif", display: "flex", alignItems: "center", gap: 5 }}><Zap size={12} /> Auto-apply</button>
                  <button onClick={(e) => { e.stopPropagation(); onOpenJob(job); }} style={{ padding: "7px 14px", borderRadius: 8, background: C.surface, border: `1px solid ${C.border}`, color: C.text, fontWeight: 600, fontSize: 12, cursor: "pointer", fontFamily: "DM Sans, sans-serif", display: "flex", alignItems: "center", gap: 5 }}><FileText size={12} /> Tailor resume</button>
                  <button onClick={(e) => e.stopPropagation()} style={{ padding: "7px 10px", borderRadius: 8, background: "transparent", border: `1px solid ${C.border}`, color: C.textDim, cursor: "pointer", display: "flex", alignItems: "center" }}><MoreVertical size={13} /></button>
                </div>
              </div>
            </div>
          </div>
